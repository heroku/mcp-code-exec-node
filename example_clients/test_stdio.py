...
def mcp(method_name, args=None):
    ...
    Examples:
        This will only work locally:
            ...
            python example_clients/test_stdio.py mcp call_tool --args '{"name": "code_exec_node", "arguments": {"code": "console.log(Array.from({length: 50}, () => Math.random()));"}}'

        To run against your deployed code:

            heroku run --app $APP_NAME -- bash -c 'python -m example_clients.test_stdio mcp list_tools'

            json='{"name": "code_exec_node", "arguments": {"code": "console.log(Array.from({length: 50}, () => Math.random()));"}}'
